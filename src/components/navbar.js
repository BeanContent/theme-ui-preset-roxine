...
import ShoppingCart from './shopping-cart-panel';
 ...
 `)
  ...
  return (
    <nav className='navbar'>
      ...
      <div style={{ display: isOpenSearch ? 'block' : 'none' }} className={isOpenSearch ? "navbar__search-container animate__animated animate__fadeIn" : "navbar__search-container"}>
        <input placeholder="Type & Press Enter" className="navbar__search" />
        <button onClick={openSearch} href="#" className="x-search icon-close-round navbar-link navbar__search-close"></button>
      </div>
      {
        isMobile ?
          <>
            ...
            <Link to='/' className="navbar__brand">
              <StaticImage className='navbar__logo dark' src='../images/logo-dark.svg' alt="logo-dark" />
            </Link>

          </>
          :
          <Link to='/' className="navbar__brand">
            <StaticImage className='navbar__logo' src='../images/logo.svg' alt="logo-light" />
          </Link>
      }
      <div style={isMobile ? { maxHeight: '0vh' } : null} className="navbar__nav">
        <ul className='navbar__nav-list'>
          {menu.map((item, index) => {
            if (item.dropdown) {
              ...
              return (
                <li key={index} className="list__item">
                  <Link to={item.path} className="list__item-link">{toUpperCase(item.name)}</Link>
                </li>
              )
            }
          })}
